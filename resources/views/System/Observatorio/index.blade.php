...
@section('page_name', 'Observatorio')
...
@section('page-header')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Observatorio</h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <form action="{{route('sistema.observatorio.store')}}" method="post">
                @csrf
                <div class="card-body">
                    <div class="mb-3">
                        <label for="titulo" class="class"></label>
                        <input type="text" id="titulo" name="titulo" class="form-control" value="@if($data){!! $data->titulo !!}@endif" placeholder="Título" required>
                    </div>
                    <div class="mb-3">
            ...
            @hasanyrole('SuperAdmin')
              @if($data && $data->active === 0)
                <form
                  action="{{route('sistema.observatorio.approve', $data->id)}}"
                  class="d-inline formapprove" method="POST">
                    @method('PUT')
                    @csrf
                    <button class="btn btn-info data" type="submit">Aprobar</button>
                </form>
              @endif
            @endhasanyrole
        </div>
