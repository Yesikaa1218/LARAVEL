...
@if($active == 0)
<form
    action="{{route('sistema.noticias.approve', $id)}}"
...
@endif
@endhasanyrole
<a href="{{route('sistema.noticias.edit', $id)}}" class="btn btn-success btn-sm" >Editar</a>
<a href="{{route('sistema.preview.noticias.mostrar-noticia', $slug)}}" class="btn btn-light" target="_blank">Ver preview</a> 
<form
    action="{{route('sistema.noticias.destroy', ['noticia' => $id])}}"
